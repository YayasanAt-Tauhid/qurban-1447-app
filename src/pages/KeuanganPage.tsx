...
const KeuanganPage = () => {
  ...
  // Total Terbayar berdasarkan filter aktif (filteredHewan)
  const totalTerbayarFiltered = filteredHewan?.reduce((sum, h) => {
    return sum + getPenjualPaymentTotal(h.id);
  }, 0) ?? 0;
  ...
  // Total Terbayar iuran berdasarkan filter aktif (filteredIuran)
  const totalTerbayarIuranFiltered = filteredIuran?.reduce((sum, s) => {
        ...
        <TabsContent value="iuran" className="space-y-6">
          {loadingIuran ? <Skeleton className="h-48 w-full" /> : (
            <>
              ...
              {/* Total Terbayar berdasarkan filter aktif */}
              <Card className="border-green-200 bg-green-50/40 dark:bg-green-950/20 dark:border-green-800">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm text-muted-foreground">
                        Total Terbayar
                        {filterBayar !== "semua" && (
                          <span className="ml-1 text-xs font-medium text-green-700 dark:text-green-400">
                            ({filterBayar === "lunas" ? "Lunas" : filterBayar === "dp" ? "DP" : "Belum Bayar"})
                          </span>
                        )}
                      </p>
                      <p className="text-xl font-bold text-green-600 dark:text-green-400">{formatRupiah(totalTerbayarIuranFiltered)}</p>
                    </div>
                  </div>
                  <p className="text-xs text-muted-foreground">{filteredIuran?.length ?? 0} shohibul</p>
                </CardContent>
              </Card>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm text-muted-foreground">
                        ...
                        {filterPenjual !== "semua" && (
                          <span className="ml-1 text-xs font-medium text-green-700 dark:text-green-400">
                            ({filterPenjual === "lunas" ? "Lunas" : filterPenjual === "dp" ? "DP" : "Belum Bayar"})
                          </span>
                        )}
                      </p>
                      <p className="text-xl font-bold text-green-600 dark:text-green-400">{formatRupiah(totalTerbayarFiltered)}</p>
                    </div>
                  </div>
                  <p className="text-xs text-muted-foreground">{filteredHewan?.length ?? 0} hewan</p>
                </CardContent>
              </Card>
